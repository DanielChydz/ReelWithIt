...
def test_user_token(test_user: schemas.UserOut, session: Session) -> schemas.Token:
    user: models.Users | None = (
        ...
        .filter(models.Users.username == test_user.username)
        .first()
    )
...
@pytest.fixture
def test_movie(authorized_client: TestClient) -> schemas.MovieOut:
...
def clear_db_override() -> Generator[None, None, None]:
    yield
