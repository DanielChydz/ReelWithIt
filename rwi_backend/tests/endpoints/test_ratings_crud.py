...
def test_add_rating_success(
    authorized_client: TestClient, test_movie: schemas.MovieOut
) -> None:
    rwi_response: Response = authorized_client.post(
        f"/rating/{test_movie.movie_id}", json={"rating": 8}
    )
...
def test_add_rating_failure(
    authorized_client: TestClient, test_movie: schemas.MovieOut, rating: float
) -> None:
    rwi_response: Response = authorized_client.post(
        f"/rating/{test_movie.movie_id}", json={"rating": rating}
    )
    assert rwi_response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT


def test_add_rating_duplicate(
    authorized_client: TestClient, test_movie: schemas.MovieOut
) -> None:
    authorized_client.post(f"/rating/{test_movie.movie_id}", json={"rating": 5})
    rwi_response: Response = authorized_client.post(
        f"/rating/{test_movie.movie_id}", json={"rating": 6}
    )
    assert rwi_response.status_code == status.HTTP_409_CONFLICT


def test_add_rating_movie_not_found(authorized_client: TestClient) -> None:
    rwi_response: Response = authorized_client.post(
        f"/rating/9999999", json={"rating": 7}
    )
    assert rwi_response.status_code == status.HTTP_404_NOT_FOUND


def test_add_rating_unauthorized(
    unauthorized_client: TestClient, test_movie: schemas.MovieOut
) -> None:
    rwi_response: Response = unauthorized_client.post(
        f"/rating/{test_movie.movie_id}", json={"rating": 5}
    )
    assert rwi_response.status_code == status.HTTP_401_UNAUTHORIZED


def test_read_rating_success(
    authorized_client: TestClient, test_movie: schemas.MovieOut
) -> None:
    authorized_client.post(f"/rating/{test_movie.movie_id}", json={"rating": 5})
...
def test_read_rating_movie_not_found(authorized_client: TestClient) -> None:
...
def test_update_rating_success(
    ...
    authorized_client.post(f"/rating/{test_movie.movie_id}", json={"rating": 5})
    rwi_response: Response = authorized_client.patch(
        f"/rating/{test_movie.movie_id}", json={"rating": 9}
    )
    assert rwi_response.status_code == status.HTTP_200_OK
...
def test_delete_rating_unauthorized(
    ...
    rwi_response: Response = unauthorized_client.delete(
        f"/rating/{test_movie.movie_id}"
    )
    assert rwi_response.status_code == status.HTTP_401_UNAUTHORIZED
