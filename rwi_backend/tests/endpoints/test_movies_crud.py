...
def test_create_movie_success(
    ...
    new_movie: dict[str, Any] = {"title": "title", "year": 2000, "director": "director"}
...
def test_create_movie_failure(
    ...
    """Parametrized invalid-create test"""
    new_movie: dict[str, Any] = {"title": title, "year": year, "director": director}
    rwi_response: Response = authorized_client.post("/movie", json=new_movie)
...
def test_create_movie_unauthorized(unauthorized_client: TestClient) -> None:
    new_movie: dict[str, Any] = {"title": "title", "year": 2000, "director": "director"}
...
def test_update_movie_unauthorized(
    ...
    new_movie: dict[str, Any] = {"title": "title", "year": 2000, "director": "director"}
    rwi_response: Response = unauthorized_client.put(
        f"/movie/{test_movie}", json=new_movie
    )
    assert rwi_response.status_code == status.HTTP_401_UNAUTHORIZED


def test_get_movie(authorized_client: TestClient, test_movie: schemas.MovieOut) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "title", 2000, "director", status.HTTP_409_CONFLICT, id="movie exists"
        ),
    ],
)
def test_update_movie(
    ...
    updated_movie: dict[str, Any] = {"title": title, "year": year, "director": director}
    rwi_response: Response = authorized_client.put(
        f"/movie/{test_movie.movie_id}", json=updated_movie
    )
...
def test_delete_movie_authorized(
    ...
    rwi_response = authorized_client.delete(f"/movie/{test_movie.movie_id}")
