...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    ...
    if username != current_user.username:
        raise HTTPException(
            ...
            detail=f"User {current_user.username} has no permission to update user {username}",
        )

    existing_data: models.Users | None = (
        db.query(models.Users)
        .filter(
            or_(
                models.Users.username == user.username, models.Users.email == user.email
            ),
            models.Users.user_id != existing_user.user_id,
        )
        .first()
    )
    if existing_data:
        if existing_data.email == user.email:
            raise HTTPException(
                ...
                detail=f"Can't change email to {user.email} - already taken",
            )
        if existing_data.username == user.username:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Can't change username to {user.username} - already taken",
            )
...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    if username != current_user.username:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to delete another user",
        )
