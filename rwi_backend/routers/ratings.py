...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    ...
    existing_rating: models.Ratings | None = (
        db.query(models.Ratings)
        .filter_by(user_id=current_user.user_id, movie_id=movie_id)
        .first()
    )
    if existing_rating:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"User with id {current_user.user_id} has already rated movie with id {movie_id}",
        )
    new_rating = models.Ratings(
        user_id=current_user.user_id, movie_id=movie_id, rating=rating.rating
    )
    db.add(new_rating)
    existing_movie.rating = AddRating(
        existing_movie.rating, existing_movie.number_of_ratings, rating.rating
    )
...
@router.get(
    "/{movie_id}", status_code=status.HTTP_200_OK, response_model=schemas.RatingOut
)
def read_rating(
...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    ...
    if existing_rating is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Rating with id {current_user.user_id} has not rated movie with id {movie_id}",
        )

    existing_movie.rating = UpdateRating(
        existing_movie.rating,
...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    ...
    existing_movie.rating = RemoveRating(
        existing_movie.rating, existing_movie.number_of_ratings, existing_rating.rating
    )
    existing_movie.number_of_ratings -= 1
    db.commit()
