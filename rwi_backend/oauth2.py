...
>>>>>>> cf9d1b3 (Added refresh tokens, improved readibility)
    ...
    encoded_jwt = jwt.encode(
        to_encode, settings.SECRET_KEY, settings.ENCODING_ALGORITHM
    )
...
def get_current_user_auth(
    ...
    user: models.Users | None = (
        ...
        .filter(models.Users.user_id == user_token_data.user_id)
        .first()
    )
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User with id {user_token_data.user_id} not found - account banned, removed or never existed",
        )
